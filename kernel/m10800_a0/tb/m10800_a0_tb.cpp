...
int main() {
    ...
    m10800_a0(input, output_hw, match_index, size, mode, golden_dummy, salt, salt_position, use_salt);
    ...
    int target_idx = 500;
    ap_uint<512> golden = output_hw[target_idx];
    ...
    std::cout << "Target hash (with salt): 0x" << print_hash(extract_sha384(golden)) << "\n";
    
    // Run kernel in search mode
    mode = 1;
    m10800_a0(input, output_hw, match_index, size, mode, golden, salt, salt_position, use_salt);

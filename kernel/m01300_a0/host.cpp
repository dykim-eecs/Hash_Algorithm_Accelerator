...
// ============================================================================
// NUM_CORE and MAX_LEN are now defined in m01300_a0.hpp
 ...
 */
static inline std::string to_sha224_hex(const ap_uint<512>& hash) {
    ...
    if (words.size() <= LARGE_DATASET_THRESHOLD) {
        ...
        for (size_t i = 0; i < words.size(); i++) {
            ...
            std::cout << "[" << std::setw(6) << i << "] " << words[i] 
                      << " -> 0x" << to_sha224_hex(hash) << std::endl;
        }
    } else {
...
void save_results_to_file(const std::vector<std::string>& words,
    ...
    for (size_t i = 0; i < words.size(); i++) {
        ap_uint<512> hash = output_data[i];
        out_file << words[i] << " 0x" << to_sha224_hex(hash) << "\n";
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        // ====================================================================
        std::vector<std::string> cu_instance_names = {"m01300_a0_1", "m01300_a0_2", "m01300_a0_3"};
        ...
        for (int i = 0; i < NUM_CUS; ++i) {
            try {
                std::string full_kernel_name = "m01300_a0:{" + cu_instance_names[i] + "}";

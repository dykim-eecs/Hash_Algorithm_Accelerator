...
// ============================================================================
#define NUM_CUS 3
#define WORD_SIZE 48 // SHA-384 digest = 48 bytes
...
// NUM_CORE and MAX_LEN are now defined in m10800_a3.hpp
// ============================================================================
// Utility Functions
// ============================================================================
/**
 * Convert ap_uint<384> hash to uppercase hexadecimal string
 */
static inline std::string to_sha384_hex(const ap_uint<384>& hash) {
...
/**
 * Parse hexadecimal string to ap_uint<384>
 * Input must be exactly 96 hex characters (384 bits)
 */
ap_uint<384> parse_hex_to_512(const std::string& hex_str) {
    ...
    for (int i = 0; i < 48; ++i) {
        ...
        result.range(383 - i * 8, 376 - i * 8) = byte_val;
    }
    return result;
}
...
// Result Processing
// ============================================================================
void print_results_mode0(const std::vector<std::vector<ap_uint<384>>>& output_datas,
        ...
        for (size_t cu = 0; cu < NUM_CUS; ++cu) {
            ...
            for (size_t j = 0; j < part_hashes.size(); ++j) {
                ...
                std::cout << "[" << std::setw(6) << global_i << "] "
                          << " -> 0x" << to_sha384_hex(part_hashes[j]) << std::endl;
            }
        }
    } else {
...
void save_results_to_file(const std::vector<std::vector<ap_uint<384>>>& output_datas,
    ...
    for (size_t cu = 0; cu < NUM_CUS; ++cu) {
        const auto& part_hashes = output_datas[cu];
        for (size_t j = 0; j < part_hashes.size(); ++j) {
            out_file << "candidate_" << (processed) << " 0x" << to_sha384_hex(part_hashes[j]) << "\n";
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        ap_uint<384> golden = 0;
        if (args.mode == 1) {
            golden = parse_hex_to_512(args.golden_hex);
            std::cout << "Golden hash: 0x" << to_sha384_hex(golden) << std::endl;
        }
        ap_uint<512> golden_word = pack_sha384(golden);
        ...
        // Step 4: Load kernel
        // ====================================================================
        std::vector<std::string> cu_instance_names = {"m10800_a3_1", "m10800_a3_2", "m10800_a3_3"};
        std::vector<int> banks = {0, 2, 3};
        std::vector<cl::Kernel> kernels(NUM_CUS);
        for (int i = 0; i < NUM_CUS; ++i) {
            std::string full_kernel_name = "m10800_a3:{" + cu_instance_names[i] + "}";
        ...
        if (args.mode == 0) {
            ...
            std::vector<std::vector<ap_uint<384>>> output_datas(NUM_CUS);
            for (int i = 0; i < NUM_CUS; ++i) {
                ...
                for (size_t j = 0; j < word_count; ++j) {
                    output_datas[i][j] = extract_sha384(raw[j]);
                }
            }
